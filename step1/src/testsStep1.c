...
int test_memoire(mips* arch, unsigned int adresse, unsigned int* addr_in_block){
	...
	if(adresse<arch->segment[BSS].start_addr+arch->segment[BSS].size && adresse>=arch->segment[BSS].start_addr){
	*addr_in_block=adresse-arch->segment[BSS].start_addr;
	return BSS;
	}

	if(adresse<arch->segment[TEXT].start_addr+arch->segment[TEXT].size && adresse>=arch->segment[TEXT].start_addr){
	*addr_in_block=adresse-arch->segment[TEXT].start_addr;
	return TEXT;
	}	
	
	if(adresse<arch->segment[DATA].start_addr+arch->segment[DATA].size && adresse>=arch->segment[DATA].start_addr){
	*addr_in_block=adresse-arch->segment[DATA].start_addr;
	return DATA;
	}
