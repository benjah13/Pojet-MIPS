...
/* fonction init_mips - renvoie une architecture MIPS */
void init_mips(mips* arch)
{
	/* initialisation des segments mémoire */
...
/* fonction init-segment - renvoie une section ELF initialisée */
...
int execute_cmd_dm(int cas, unsigned int addr, unsigned int arg2, mips* arch)
{
	...
	switch(test_memoire(arch,addr,&addr_in_block)){
		...
		case STACK:
		valeur=arch->segment[STACK].data[addr_in_block];
		fprintf(stdout,"%08x:	%02x\n", addr, valeur);
		break;
		}
...
int execute_cmd_dr(int test_reg, mips* arch)
{
	...
	/* affichage de tous les registres */
		...
		for(i=0;i<36;i++){
			fprintf(stdout,"$%s	$%d	%d	0x%x\n" ,arch->reg[i].mnemo,arch->reg[i].indice,arch->reg[i].val,arch->reg[i].val);
		}
	}
	...
	/* erreur de notation */ 
		...
		fprintf(stdout,"$%s	$%d	%d	0x%x\n" ,arch->reg[test_reg].mnemo,arch->reg[test_reg].indice,arch->reg[test_reg].val,arch->reg[test_reg].val);
	}
