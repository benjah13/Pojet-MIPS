...
/*******************************************************/
/******* GENERATION DE LA PILE (facultatif) ************/
/*******************************************************/

/* fonction initStack - initialise la pile */
void initStack(mips* arch)
{
	arch->segment[STACK].name=".stack";
	arch->segment[STACK].size=0x1000;
	arch->segment[STACK].data=calloc(arch->segment[STACK].size,sizeof(BYTE));;
	arch->segment[STACK].startAddress=0x3000;

	/* Initialisation du pointeur SP sur le sommet de la pile */
	arch->reg[29].val=arch->segment[STACK].startAddress+arch->segment[STACK].size-4;
}
	...
	while(i<nbr_instr && j<arch->segment[TEXT].startAddress+arch->segment[TEXT].size){
		...
		else
		{
			...
			do
			{
				...
				if(strcmp("rs",arg)==0)
				{
					fprintf(stdout,"$%s ",arch->reg[rs].mnemo);
				}
		
				else if(strcmp("rs,",arg)==0)
				{
					fprintf(stdout,"$%s, ",arch->reg[rs].mnemo);
				}
				...
				{
					fprintf(stdout,"$%s ",arch->reg[rt].mnemo);
				}
				...
				else if(strcmp("rd",arg)==0)
				{
					fprintf(stdout,"$%s ",arch->reg[rd].mnemo);
				}
				...
				else if(strcmp("offset",arg)==0)
				{
					fprintf(stdout,"%x ",attribut);
				}
		
				else if(strcmp("offset(base)",arg)==0)
				{
					fprintf(stdout,"%x($%s)",attribut,arch->reg[rs].mnemo);
				}

				else if(strcmp("immediate",arg)==0)
				{
					fprintf(stdout,"%d ",(int16_t)attribut);
				}
				...
				else if(strcmp("<branch>",arg)==0)
				{
					if(getAddressName(addr+4*i+(attribut<<2)+4)!=NULL)
					{
						fprintf(stdout,"<%s>",getAddressName(addr+4*i+(attribut<<2)+4));
					}
				}

				else if(strcmp("<jump>",arg)==0)
				{
					if(getAddressName(attribut<<2)!=NULL)
					{
						fprintf(stdout,"<%s>",getAddressName(attribut<<2));
					}
				}
			}
